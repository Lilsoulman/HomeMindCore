...
> **当前阶段：** V2.9 剪辑体验重构 + 主体功能「主动式家庭管家」确认（承接 V2.8 四引擎调度）。
> **版本语义：** V2.9 = V2.8 + 剪辑体验重构（B37-B40）+ 主体功能确认（§1.1 主动式家庭管家：早晚汇报/异常主动发现/对话式管家，地基为家庭上下文引擎与主动推送）+ Web 学习记忆库规划（M3/W-M3）。剪辑等 Skill 是「手脚」，主体是「大脑」；两者并行推进。
> **最后更新：** 2026-08-14
...
### 1.1 主体功能：主动式家庭管家（V2.9 确认）

**主体功能定义：** NexusMind 不是“一堆 Skill 的集合”，而是一个 **主动式家庭管家（Proactive Family Steward）**——AI 是家庭的一员，持续理解家庭上下文（成员、日程、设备、环境、习惯、内容），在需要时主动开口，在授权时受控行动。剪辑、思维导图、美团、日程、设备控制等所有 Skill 都是它的“手脚”（能力），不是产品主体；主体是“AI 持续理解这个家并主动服务”。

**三个最小可感知形态（用户每天能体验到的智能体）：**

| 形态 | 用户感知 | 关键依赖 |
| --- | --- | --- |
| **早晚汇报（Briefing）** | 每天 7:30 / 21:00 主动推送：“今早 16°，爸爸 9 点有早班机，老人房温度偏低 2°，有 3 件事等你确认” | 主动推送通道（未落地）+ 家庭上下文聚合 |
| **异常主动发现（Proactive Alerting）** | AI 主动发现“门 2 小时没关”“热水器今天多耗 3 度”“爸爸连续 3 天凌晨醒”，不等用户打开 App 就开口 | 家庭事件流（未落地）+ 现有数据 |
| **对话式管家（Conversational Steward）** | 一句话收口一切：“今天家里怎么样？”“下周末安排得怎样了？”“把上周杭州的素材剪了”——日程/设备/记忆/剪辑/美团全在对话里 | 已有专家会话（B20）+ 各 Skill 接入对话入口 |

**两个技术地基（主体功能成立的前提，均为软件能力、可复用现有资产）：**

1. **家庭上下文引擎（轻量版）**：定时聚合各域数据（日程/天气/设备/成员/事件）生成“家庭状态快照”，供汇报、异常检测与对话消费。现有 Dashboard 的 `homeSummary`/`pendingConfirmations`/`stewardActivities` 是雏形，需从“拉取式”演进为“聚合式上下文”。成本：小-中。
2. **主动推送通道（最大缺口）**：产品 §3 推送聚合策略（L3>L2>周期摘要>场景聚合>L1、静默偏好、防跨家庭泄露）已有完整设计但零实现；没有推送，“主动”不成立。成本：中（FCM/厂商通道 + 分级送达 + 偏好设置）。

**与既有能力的边界：** 剪辑（B37-B40）等 Skill 是“手脚”先行——先证明智能体有能干的手；主体功能是“大脑”——上下文引擎 + 推送 + 汇报/异常/对话。两者并行推进，剪辑不阻塞主体，主体不阻塞剪辑；主体功能的验收以“用户每天收到一条有家庭上下文的主动消息并愿意读”为准。

**验证场景路线（2026-08-14 确认）：** 主体功能的验证按"轻 → 重"推进，垂直场景只用于验证大脑，不是产品本身。手脚先行（剪辑 + 管家功能矩阵）验证 AI 能力；终极形态后置。

1. **手脚 1：剪辑（B37-B40）**——验证"AI 可信执行"（产物立等可见，已排期）；
2. **手脚 2：管家功能矩阵（8 个完整功能，B41 起排期）**——每个都是可独立立起的完整闭环（用户表达 → AI 主动行为 → 确认动作），互不重复地验证"主动"的不同侧面：

| # | 管家功能 | 一句话 | 验证"主动"的侧面 | 数据源 | 成本 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 家庭财务管家 | AI 每周分析收支，主动给省钱建议 | 主动分析+建议 | 支付宝/微信导出 CSV 或截图 OCR（无需硬件） | 中 | ✅ 已确认 |
| 2 | 家庭周末出游管家 | "下个月带爸妈去杭州，两晚，预算四千"→查美团酒旅→方案确认→外跳支付 | 上下文→真实生活服务闭环 | 美团 travel skill（MTR-1a 待验） | 中 | ✅ 设计已存在（§7.3.3） |
| 3 | 快递管家 | 全自动接快递动态，"包裹今天到但家里没人→改投驿站" | 主动发现+即时价值 | 快递100 官方 MCP | 中-小 | ✅ 已确认 |
| 4 | 家庭日程协同管家 | 多成员日历交叉：冲突检测/空档建议/缴费证件到期提醒 | 跨成员交叉发现 | 成员日历（已有） | 小 | ✅ 已确认 |
| 5 | 家庭回忆管家 | 照片视频自动归集分类，每月生成"家庭回忆"短视频 | 手脚升级为主动服务 | 手机相册 + 剪辑四引擎（B37-B40） | 中 | ✅ 已确认 |
| 6 | 宠物管家 | 记住宠物档案+疫苗/驱虫日历+消耗速度，"7 天后断粮要下单吗" | 长期关怀+补货预测 | 手动建档 + 财务 CSV 消耗数据 | 小-中 | ✅ 已确认 |
| 7 | 缴费管家 | 管住水电燃气+物业+话费+保险到期日历，提前提醒永不逾期 | 到期预防+不逾期 | 缴费单 OCR + 财务 Agent 账单数据共享 | 小 | ✅ 已确认 |
| 8 | 家庭健康关注管家 | 自建家庭健康画像（基础档案/健康日历/报告库），体检/疫苗/复查到期提醒、指标趋势 | 长期关怀+趋势发现 | 自建画像（本地存储）+ 安诊儿/医院报告截图 PDF 导入（用户主动提供，非系统对接） | 中 | ✅ 已确认 |

3. **终极场景：智能家居（主动式家庭管家完整形态）**——AI 理解层的完整展示（设备厂商只有"物"的数据，NexusMind 有"人"的数据），因依赖硬件生态与 H4 写入，**降级为长期愿景后置推进，不抛弃**。

北极星指标不变：AI 完成的有效任务中"主动发现并解决"占比持续上升。
...
移动端（Flutter）的信息架构、设计系统、页面交互和实施计划以本文 §3 的 Flutter 规范及对应移动端实施文档为准。Web 端（Vue 2 + Element UI）的信息架构、PC 交互、路由、组件、API 映射和实施计划以 `D:\HomeMind\admin\docs\main\NexusMind-Web-Development.md`、`D:\HomeMind\admin\docs\main\NexusMind-Web-Development-Plan.md` 为准。两端共用产品、领域、权限、风险、审计和 `/api/v1` 契约；任一端的页面或计划变更都不得被误写为另一端的实施状态。
...
| 入口 | 主要用户 | 产品职责 | 明确边界 |
| --- | --- | --- | --- |
| Web 用户端 | 所有已登录家庭成员 | 家庭概览、确认中心、管家动态、成员/知识、学习记忆库、个人偏好、本人连接、本人技能与 Run 记录 | 不配置家庭凭据，不查看其他成员个人连接 |
...
**Web 学习记忆库（M3/W-M3）：** 参考 Hermes Studio 的记忆可见性，Web 用户端提供独立路由 `/app/memories` 展示 AI 已学习且**已接受、可召回**的记忆。它是面向成员理解和纠正 AI 长期学习结果的只读优先视图，不是 Prompt、完整会话、模型思考过程或 N97 派生索引查看器。页面按个人记忆与家庭记忆、偏好/事实/决策等类型、更新时间和来源筛选；卡片仅显示可见摘要、作用域、置信度/稳定性、最近学习时间、来源 Run/Conversation 的脱敏引用及失效/已归档状态。个人记忆仅本人可见；家庭记忆遵守 `family.read` 与成员角色；无权引用只显示受限计数，不泄露原文、其他成员身份或外部凭据。M2 的「记忆候选」继续位于家庭知识页，专用于接受、编辑后接受或拒绝；学习记忆库只展示候选被接受后写入的事实源及其学习溯源，两者不能混合。
...
**实施顺序（V2.5 HA 主线）**：`H1 MCP 会话基线 → H2 HA MCP 只读发现/状态 → H3 WebSocket 实时同步 → H4 受控设备写入与确认 → H5 审批体验增强 → M1 Context Snapshot → M2 Memory Candidate → M3 学习记忆库`。M3 在 M2 已能把候选受控写入事实源后，发布脱敏、按可见性隔离的查询与溯源契约，供 Web `/app/memories` 展示；HA Token 只保存在 N97 Secret/Vault；任何写操作超时均进入 `result_unknown` 并通过状态回读确认，不盲目自动重试。
...
#### 7.1.1 V2.9 剪辑体验重构（B37-B40，2026-08-14 设计基线）

**背景与目标**：既有快速剪辑链路已完整（素材登记 → chat 引导 → 方案 → 确认 → .draft），但产品自评存在四个体验断点：交付物是剪映草稿而非可预览视频、素材需手动上传、对话为模板问卷、仅 Web 端可操作。V2.9 以「用户丢一个素材文件夹 → 说一句话 → 拿回一段能直接看的视频」为重构目标，保持既有确认/幂等/审计链路与风险等级（L1）不变。

**体验重构四切片（量变积累，形成「剪辑好用」质变）**：

| 切片 | 能力 | 产品体验变化 | 承载端 |
| --- | --- | --- | --- |
| B37 粗剪视频产出 | 方案确认后由 ffmpeg 按方案时间线直接渲染可预览 mp4；`.draft` 降级为「去剪映精剪」进阶选项 | 交付物从「帮你开了个头」变为「帮你剪好了」 | Web 端预览/下载 |
| B38 素材自动发现 | 配置素材根目录（本机/NAS），后台周期扫描新素材自动登记（ffprobe 元数据、去重） | 素材第一公里从手动上传变为自动出现 | Web 端素材区展示 |
| B39 自然语言对话解析 | chat 升级为真 LLM 解析：一句话（「剪成 30 秒竖屏快节奏带字幕」）→ 结构化参数 → 直接生成方案；AI 不可用时降级回模板问卷 | 从问卷对话变为说人话 | Web 端对话区 |
| B40 移动端轻量入口 | 移动端新增剪辑入口：选素材/相册文件 → 一句话请求 → 任务进度 → 完成预览；方案时间线/修改/导出仍仅 Web 端 | 手机拍完素材即可发起，打破「只在 Web 端」 | 移动端（Flutter） |

**跨端分级修订（承接既有「Skill 跨端分级」决策）**：快速剪辑仍属复杂 Skill（产物渲染/下载在 Web 端），但新增「移动端轻量触达」例外——移动端只承载素材选择、一句话请求、任务进度与完成预览（均为只读/发起动作），不承载方案时间线、修改指令与导出；该例外的分级判定仍以产物形态为准，不引入新元数据字段。

**B37 粗剪 mp4 契约要点**：渲染为异步后台任务，`clipping_tasks.status` 增加 `rendering` 阶段（`generating → reviewing → rendering → done|failed`）；mp4 产物经既有 `RegisterGeneratedFileAsync` 登记为生成文件（复用 readToken 下载）；渲染失败写 `failed` 事件且可重试，不伪造成功；首版仅 concat+trim+转码，不引入滤镜与 GPU。素材须为本地可访问路径（上传或扫描登记），渲染产物路径不对外暴露。
...
**B40 移动端入口要点**：移动端「能力」或首页快捷入口新增剪辑发起：素材选择（相册/文件，单文件 ≤500MB）、一句话输入、任务创建（复用既有 `clipping/chat` + `runs` 链路）、进度轮询（复用公开事件，不消费引擎内部数据）、完成后 Web 或 App 内预览 mp4；断网/失败有降级提示。

**默认关闭与验收门禁**：B37 的 ffmpeg 渲染默认关闭，配置 `Clipping:Render` 且 `ffmpeg` 可执行（`tools/ffmpeg/bin/ffmpeg.exe` 已就绪）后启用；B39 依赖 AI 配置启用；四引擎（video-use/Seedance/HyperFrames/Remotion）仍保持关闭，本次重构不改变其门禁。各切片验收标准见后端开发计划 B37-B39 与 Web/移动端计划切片。
...
| --- | --- | --- | --- | --- | --- |
| 2026-08-14 | 管家功能矩阵确认（手脚 2 扩展） | §1.1「验证场景路线」手脚 2 由单一「家庭财务 Agent」扩展为 **8 个管家功能矩阵**（财务/出游/快递/日程协同/回忆/宠物/缴费/健康画像），每个均为完整闭环（用户表达 → AI 主动行为 → 确认动作），互不重复验证「主动」的侧面；新增家庭健康关注管家：以**自建家庭健康画像**为地基（本地存储、覆盖全家成员），安诊儿/医院报告截图 PDF 作数据源（用户主动导入、非系统对接；用户位于浙江余姚，安诊儿为浙江省卫健委+支付宝 AI 医疗应用）；原健康「老人陪护」定位取消（长辈年轻、无真实对象）；宠物/缴费管家新确认，健康画像管家回归 | 待同步 Web/移动端（管家功能按端分级接入对话与确认中心；健康画像本地化展示） | 待同步后端总设计与开发计划（B41 起排期，财务先行） | 已同步设计，切片待排期 |
| 2026-08-14 | 验证场景路线确认（归档） | §1.1 新增「验证场景路线」：手脚 1 剪辑（B37-B40，验证可信执行）→ 手脚 2 家庭财务 Agent（验证主动分析+建议，数据源支付宝/微信 CSV 或 OCR，30 天内可见价值，节能为其子集）→ 终极场景智能家居（降级为长期愿景后置，不抛弃）。B37 已由维护者实现（FfmpegRenderService 默认关闭、渲染 202 异步、失败置失败、248/248 测试通过） | 剪辑 B37-B40 跨端切片已排期 | B37 已实施；B38/B39 待实施 | 已同步设计，B37 已实现 |
| 2026-08-14 | Web 学习记忆库实现 | Web `/app/memories` 展示已接受、可召回的个人/家庭学习记忆及脱敏溯源；候选审核仍留在家庭知识页。显式 Run 候选经后台 Worker 进入待审核队列，不从 Prompt、会话或自由摘要推断 | 已同步 Web 总设计与 W-M3 开发计划 | M3 已实现候选审核、事实源/投影原子写入、只读 API 与受控 Worker；真实 MySQL 迁移待部署验证 | 已实现，待部署验收 |
| 2026-08-14 | 主体功能确认：主动式家庭管家 | 新增 §1.1「主体功能：主动式家庭管家」：明确剪辑/美团/思维导图等 Skill 是「手脚」，主体是「AI 持续理解家庭并主动服务」；三个最小可感知形态（早晚汇报/异常主动发现/对话式管家）与两个技术地基（家庭上下文引擎、主动推送通道）；§9 战略目标与 Phase 3 同步对齐（Phase 3 即主体功能完整落地），北极星指标补充「主动发现并解决」占比 | 待同步移动端设计（管家 Tab 朝向汇报/异常/对话演进）与 Web 端（对话式管家入口）；剪辑 B37-B40 跨端切片已排期 | 待同步后端设计（上下文引擎与推送为后续切片，先于 B37-B39 剪辑落地）；剪辑 B37-B39 已排期 | 已同步设计，主体功能切片待排期 |
| 2026-08-14 | V2.9 剪辑体验重构 | 新增 §7.1.1「剪辑体验重构（B37-B40）」：目标「丢素材文件夹 → 说一句话 → 拿回可预览视频」；B37 粗剪 mp4 产出（ffmpeg 渲染、`.draft` 降级进阶选项、`rendering` 阶段）、B38 素材自动发现（素材根目录扫描、`source_type`/`directory_key`）、B39 自然语言对话解析（LLM → 结构化参数，AI 禁用降级模板问卷）、B40 移动端轻量入口（素材选择/一句话/进度/预览，方案时间线等仍仅 Web）；跨端分级新增「移动端轻量触达」例外；四引擎门禁不变、ffmpeg 渲染默认关闭 | 已同步 Web 端文档（工作台预览/扫描素材/LLM 解析确认卡）与移动端设计（轻量入口边界修订） | 已同步后端总设计（§16 V2.9 演进设计）与开发计划（B37-B39 排期） | 已同步设计，待实施 |
...
**主体功能主线（V2.9 确认）：** 产品的质变点是「主动式家庭管家」（§1.1）——不是功能数量堆叠，而是让 AI 持续理解家庭上下文并主动服务。剪辑（B37-B40）、美团、思维导图、设备控制都是主体功能的「手脚」，用于先验证 AI 的可信执行能力；主体功能的「大脑」由家庭上下文引擎与主动推送两个地基支撑，落地顺序为：上下文引擎 → 推送通道 → 早晚汇报/异常主动发现/对话式管家。北极星指标围绕主体功能定义：**AI 完成的有效任务中，属于「主动发现并解决」的比例应持续上升。**
...
从“用户说打开空调”升级为“结合室温、在家状态、时间和习惯，建议开启舒适模式”。新增 Family Context Engine，统一时间、天气、成员、设备、习惯和日程上下文；在明确授权下生成预测建议和自动化。**本阶段即主体功能「主动式家庭管家」（§1.1）的完整落地**：上下文引擎（地基 1）+ 主动推送（地基 2）→ 早晚汇报、异常主动发现、对话式管家三形态全部可用；剪辑（B37-B40）等 Skill 在 Phase 0-2 先行验证「手脚」，本阶段全部收口进对话式管家入口。
