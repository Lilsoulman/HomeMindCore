...
public sealed class ExpertCatalogServices : IExpertCatalogServices
{
    ...
    public async Task<ServiceResult> ListAsync(long userId, long tenantId, string? query, string? category, string? type, CancellationToken cancellationToken = default)
    {
        var experts = await (from expert in _db.Experts
                             join version in _db.ExpertVersions on expert.Id equals version.ExpertId
                             where expert.Status == "active" && version.Status == "published" && (expert.TenantId == 1 || expert.TenantId == tenantId)
                                   && (category == null || expert.Category == category)
                                   && (query == null || expert.Name.Contains(query) || expert.Code.Contains(query))
                             select new { CatalogType = "expert", expert.Id, expert.Code, expert.Name, expert.Category, expert.Description, version.EstimatedCredits }).ToListAsync(cancellationToken);
        var groups = await (from expertGroup in _db.ExpertGroups
                            ...
                            select new { CatalogType = "group", expertGroup.Id, expertGroup.Code, expertGroup.Name, expertGroup.Category, expertGroup.Description, version.EstimatedCredits }).ToListAsync(cancellationToken);
        object result = type == "expert" ? experts : type == "group" ? groups : experts.Cast<object>().Concat(groups);
    ...
    public async Task<ServiceResult> GetAsync(long userId, long tenantId, long expertId, string type, CancellationToken cancellationToken = default)
    {
        ...
        var expert = await (from item in _db.Experts
                            ...
                            where item.Id == expertId && item.Status == "active" && version.Status == "published" && (item.TenantId == 1 || item.TenantId == tenantId)
                            orderby version.Version descending
                            select new { item.Id, item.Code, item.Name, item.Category, item.Description, item.PrivacyScope, VersionId = version.Id, version.Version, version.Persona, version.Methodology, version.PromptTemplate, version.ToolPolicy, version.OutputSchema, version.EstimatedCredits }).FirstOrDefaultAsync(cancellationToken);
